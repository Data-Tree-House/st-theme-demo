...
def text_card():
    ...
    st.markdown(
        r"Markdown: **bold** *italic* ~strikethrough~ [link](https://streamlit.io) `code` $\int_a^b f(x)$ 🐶 :cat: :material/home: :streamlit: <- -> <-> -- >= <= ~= "
    )
...
def auth_card():
    st.page_link("auth.py", label="Auth", icon=":material/lock:")
